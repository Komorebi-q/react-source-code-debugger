...
// only though.
const ReactSharedInteralsServer = (React)
...
export function getRoot(weakResponse) {
...
function processChunkDebugInfo(
  response,
  chunk,
  value,
) {
...
function createInitializedIteratorResultChunk(
  response,
  value,
  done,
) {
...
let isInitializingDebugInfo = false;
  ...
  if (debugChunk !== null) {
    ...
    try {
      if (debugChunk.status === RESOLVED_MODEL) {
        ...
        const initializedChunk = ((debugChunk));
...
function initializeElement(
  ...
  if (supportsCreateTask && stack !== null) {
    const createTaskFn = (console).createTask.bind(
      console,
...
function createElement(
  ...
  if (initializingHandler !== null) {
    ...
    if (handler.errored) {
      ...
      const erroredChunk = createErrorChunk(
        response,
        handler.reason,
      );
      if (__DEV__) {
...
function parseModelTuple(
  ...
  if (tuple[0] === REACT_ELEMENT_TYPE) {
    ...
    return createElement(
      ...
      tuple[2],
      tuple[3],
      __DEV__ ? (tuple)[4] : null,
      __DEV__ ? (tuple)[5] : null,
      __DEV__ ? (tuple)[6] : 0,
    );
  }
  return value;
...
function ResponseInstance(
  ...
  temporaryReferences,
  allowPartialStream,
  findSourceMapURL, // DEV-only
  replayConsole, // DEV-only
  environmentName, // DEV-only
  debugStartTime, // DEV-only
  debugEndTime, // DEV-only
...
function incrementChunkDebugInfo(
  ...
  if (__DEV__ && enableAsyncDebugInfo) {
    const debugInfo = streamState._debugInfo;
    const endTime = performance.now();
    const previousEndTime = debugInfo.end;
    const newByteLength = ((debugInfo.byteSize)) + chunkLength;
    if (
...
function resolveChunkDebugInfo(
  ...
  if (__DEV__ && enableAsyncDebugInfo) {
    ...
    if (response._debugIOStarted) {
      ...
      const asyncInfo = {awaited: streamState._debugInfo};
      if (chunk.status === PENDING || chunk.status === BLOCKED) {
...
function resolveModel(
  ...
  if (!chunk) {
    const newChunk = createResolvedModelChunk(
      response,
      model,
    );
    if (__DEV__) {
...
function resolveErrorModel(
  ...
  if (!chunk) {
    const newChunk = createErrorChunk(
      response,
      errorWithDigest,
    );
    if (__DEV__) {
...
const supportsCreateTask = __DEV__ && !!(console).createTask;

const fakeFunctionCache = __DEV__
  ? new Map()
  : (null);
  ...
  const taskName =
    ...
    env !== ownerEnv
      ...
      : // Some unfortunate pattern matching to refine the type.
        ...
        ? getServerComponentTaskName(((debugInfo)))
        : debugInfo.name !== undefined
          ? getIOInfoTaskName(((debugInfo)))
          : getAsyncInfoTaskName(((debugInfo)));
...
function buildFakeTask(
  ...
  const createTaskFn = (console).createTask.bind(console, taskName);
  const callStack = buildFakeCallStack(
    response,
    stack,
    env,
...
const createFakeJSXCallStackInDEV = __DEV__
...
function initializeDebugInfo(
  ...
  if (debugInfo.stack !== undefined) {
    const componentInfoOrAsyncInfo =
...
let currentOwnerInDEV = null;
function getCurrentStackInDEV() {
  if (__DEV__) {
    const owner = currentOwnerInDEV;
    if (owner === null) {
      return '';
    }
...
const replayConsoleWithCallStack = {
  react_stack_bottom_frame: function (
    ...
    currentOwnerInDEV =
      owner === null ? (response._debugRootOwner) : owner;
...
const replayConsoleWithCallStackInDEV = __DEV__
  ? // We use this technique to trick minifiers to preserve the function name.
...
function flushComponentPerformance(
  ...
  if (!isArray(root._children)) {
    ...
    const previousResult = root._children;
      ...
      if (endTimeIdx > -1) {
        ...
        for (let j = endTimeIdx - 1; j > i; j--) {
          ...
          if (typeof candidateInfo.name === 'string') {
            ...
            // $FlowFixMe: Refined.
            const componentInfo = candidateInfo;
            logComponentInfo(
              response,
              root,
              componentInfo,
...
export function close(weakResponse) {
