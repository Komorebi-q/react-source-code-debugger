...
export function startGestureTransition(
  ...
  try {
    ...
    // $FlowFixMe[prop-missing]
    ownerDocument.__reactViewTransition = transition;
    const customTimelineCleanup = []; // Cleanup Animations started in a CustomTimeline
    const viewTransitionAnimations = [];
    const readyCallback = () => {
      const documentElement = (ownerDocument.documentElement);
      // Loop through all View Transition Animations.
...
export function getCurrentGestureOffset(timeline) {
...
FragmentInstance.prototype.observeUsing = function (
  ...
  if (__DEV__) {
    if (enableFragmentRefsTextNodes) {
      ...
      traverseFragmentInstance(this._fragmentFiber, (child) => {
...
function collectClientRects(child, rects) {
  if (enableFragmentRefsTextNodes && child.tag === HostText) {
    const textNode = child.stateNode;
...
export function createFragmentInstance(
  ...
  const fragmentInstance = new (FragmentInstance)(fragmentFiber);
  if (enableFragmentRefsInstanceHandles) {
    traverseFragmentInstance(
      fragmentFiber,
...
function estimateImageBytes(instance) {
  ...
  const height = instance.height || 100;
  const pixelRatio =
...
let estimatedBytesWithinLimit = 0;
