...
export const SomeTransitionLane = TransitionLane1;
...
let nextTransitionUpdateLane = TransitionLane1;
let nextTransitionDeferredLane = TransitionLane11;
...
export function includesRetryLane(lanes) {
  return (lanes & RetryLanes) !== NoLanes;
}

export function includesIdleGroupLanes(lanes) {
  return (
    (lanes &
      (SelectiveHydrationLane |
        IdleHydrationLane |
...
export function claimNextTransitionUpdateLane() {
...
export function claimNextTransitionDeferredLane() {
...
export function getHighestPriorityLane(lanes) {
  // 取出优先级最高的 lane。lane 结构为二进制位，每一位代表一个优先级队列。
  // lanes & -lanes 返回最低有效位的那个 1，也就是最高优先级的 lane。
  // 例如，如果 lanes = 0b10100，则 -lanes = 0b01100，结果为 0b00100。
  
  // -lanes 的推导逻辑如下：
  // 在 JavaScript 和大多数编程语言中，对一个整数取负，等价于对其按位取反加一（即补码运算）。
  // 例如，假设 lanes = 0b10100（即十进制20），
  // -lanes = (~0b10100 + 1) => 0b01100（即十进制-20的二进制补码）。
  // 这样 lanes & -lanes 只会保留 lanes 最低的那个 1（也就是最低优先级）。
  // 例如： 0b10100 & 0b01100 = 0b00100（即只留下最后一个1）。
  // 这个技巧常用于快速找出某个二进制位字段的最低有效位（也就是优先级最高的 lane）。
  return lanes & -lanes;
}
