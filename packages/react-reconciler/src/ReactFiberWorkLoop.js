...
let workInProgressUpdateTask = null;
...
let pendingSuspendedCommitReason = null; // Profiling-only
let pendingDelayedCommitReason = IMMEDIATE_COMMIT; // Profiling-only
let pendingSuspendedViewTransitionReason = null; // Profiling-only
...
export function markRenderDerivedCause(fiber) {
  if (enableProfilerTimer && enableComponentPerformanceTrack) {
    if (__DEV__) {
...
export function flushPendingEffectsDelayed() {
  if (pendingDelayedCommitReason === IMMEDIATE_COMMIT) {
    pendingDelayedCommitReason = DELAYED_PASSIVE_COMMIT;
  }
...
export function pingGestureRoot(root) {
...
export function restartGestureRoot(root) {
  if (
    workInProgressRoot === root &&
    isGestureRender(workInProgressRootRenderLanes)
  ) {
